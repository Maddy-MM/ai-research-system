...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        default_username = os.environ.get("DEFAULT_USER") or getattr(
            settings, "effective_username", "admin"
        )
        default_password = os.environ.get("DEFAULT_PASS") or getattr(
            settings, "effective_password", "secret"
        )
        ...
        try:
            ...
            if not user:
                ...
                logger.info(f"Configured user '{default_username}' created.")
            else:
                user.hashed_password = hash_password(default_password)
                db.commit()
                logger.info(f"Configured user '{default_username}' credentials synced.")
        finally:
            db.close()
    except Exception as e:
        logger.error(f"Failed to initialize database or provision user: {e}")
    yield
...
FRONTEND_DIR = next(
    (d for d in CANDIDATE_FRONTENDS if os.path.isdir(d)),
    os.path.join(BASE_DIR, "..", "frontend"),
)
...
templates = (
    Jinja2Templates(directory=TEMPLATES_DIR) if os.path.isdir(TEMPLATES_DIR) else None
)
