...
from src.pipeline.agents import build_researcher_agent, build_verifier_agent, writer_chain, critic_chain, planner_chain, verifier_chain
        ...
        async with _mcp_lock:
            if _mcp_tools_cache is None:
                ...
                client = MultiServerMCPClient({
                    "researchmind_tools": {"url": settings.MCP_SERVER_URL, "transport": "streamable_http"},
                })
...
def planner_node(state: ResearchState) -> dict:
    ...
    logger.info("Step: Planner starting", extra={"request_id": state["request_id"], "iteration": state.get("iteration_count", 0)})
    ...
    result = planner_chain.invoke({"topic": state["topic"], "feedback_context": feedback_context})
    plan = result["parsed"]
    raw = result["raw"]
    tokens = raw.usage_metadata.get("total_tokens", 0) if getattr(raw, "usage_metadata", None) else 0

    step_duration = time.perf_counter() - step_start
    PIPELINE_STEP_DURATION_SECONDS.labels(step="plan").observe(step_duration)
    logger.info("Step: Planner complete", extra={"request_id": state["request_id"], "sub_questions": plan.sub_questions, "duration_s": round(step_duration, 2)})

    return {"sub_questions": plan.sub_questions, "clarifying_question": plan.clarifying_question, "tokens_used": tokens}
...
def route_after_planner(state: ResearchState):
    if state["clarifying_question"]:
        return END
    return [
        Send("researcher", {"topic": state["topic"], "request_id": state["request_id"], "sub_question": sq})
...
async def researcher_node(state: ResearchState) -> dict:
    ...
    step_start = time.perf_counter()
    logger.info("Researcher starting", extra={"request_id": state["request_id"], "sub_question": sub_question})

    tools = await _get_mcp_tools()
    researcher_agent = build_researcher_agent(tools)
    result = await researcher_agent.ainvoke({
        "messages": [("user", f"Research this sub-question thoroughly: {sub_question}")]
    })
    ...
    logger.info("Researcher complete", extra={"request_id": state["request_id"], "sub_question": sub_question, "duration_s": round(step_duration, 2)})
...
def write_node(state: ResearchState) -> dict:
    step_start = time.perf_counter()
    logger.info("Step: Writer starting", extra={"request_id": state["request_id"], "iteration": state.get("iteration_count", 0)})
    ...
    tokens = response.usage_metadata.get("total_tokens", 0) if getattr(response, "usage_metadata", None) else 0

    step_duration = time.perf_counter() - step_start
    PIPELINE_STEP_DURATION_SECONDS.labels(step="write").observe(step_duration)
    logger.info("Step: Report written", extra={"request_id": state["request_id"], "duration_s": round(step_duration, 2)})
...
def critique_node(state: ResearchState) -> dict:
    ...
    logger.info("Step: Critique complete", extra={"request_id": state["request_id"], "score": verdict.score, "issue_type": verdict.issue_type, "duration_s": round(step_duration, 2)})
...
async def verify_node(state: ResearchState) -> dict:
    ...
    result = verifier_chain.invoke({"research": combined_research, "report": state["report"]})
    outcome = result["parsed"]
    raw = result["raw"]
    tokens = agent_tokens + (raw.usage_metadata.get("total_tokens", 0) if getattr(raw, "usage_metadata", None) else 0)
    ...
    if total == 0:
        ...
        unsupported_lines = "\n".join(f"- [{c.verdict}] {c.claim}" for c in outcome.claims if c.verdict != "yes")

        summary = (
            f"{total - no_count - partial_count}/{total} claims fully supported, "
            f"{partial_count} partial, {no_count} unsupported.\n"
            + (f"\nFlagged claims:\n{unsupported_lines}" if unsupported_lines else "\nNo issues found.")
        )
    ...
    logger.info(
        ...
        extra={"request_id": state["request_id"], "unsupported": no_count, "partial": partial_count, "total": total, "duration_s": round(step_duration, 2)},
    )
...
def build_graph():
    ...
    graph.add_conditional_edges("critique", route_after_critic, {"planner": "planner", "write": "write", "end": "verify"})
